...
if not addon.settings.guild.enable then return end

local Stat = addon.stat.guild
...
Text:SetFont(addon.settings.guild.font, addon.settings.guild.font_size)
Text:SetPoint(unpack(addon.settings.guild.position))
Text:SetShadowColor(0, 0, 0)
Text:SetShadowOffset(1, -1)
if addon.settings.guild.class_color then
...
local function Update(self, event, ...)	
	...
	if IsInGuild() then
		...
		if (totalOnline == 1) then
			Text:SetFormattedText(displayString, 'Guild', 0)
		else
...
Stat:SetScript('OnEnter', function(self)
	...
	GameTooltip:SetOwner(self, unpack(addon.settings.guild.tooltip_position))
	GameTooltip:ClearLines()
	GameTooltip:AddDoubleLine(string.format(guildInfoString, GetGuildInfo('player'), GetGuildLevel()), string.format(guildInfoString2, 'Guild', online, #guildTable),tthead.r,tthead.g,tthead.b,tthead.r,tthead.g,tthead.b)
	GameTooltip:AddLine(' ')
	...
	if standingID ~= 4 then -- Not Max Rep
		...
		barMin = 0
		GameTooltip:AddLine(string.format('%s:|r |cFFFFFFFF%s/%s (%s%%)',col..COMBAT_FACTION_CHANGE, addon.ShortValue(barValue), addon.ShortValue(barMax), math.ceil((barValue / barMax) * 100)))
	end
