...
if not addon.settings.friend.enable then return end

local Stat = addon.stat.friend
...
Text:SetFont(addon.settings.friend.font, addon.settings.friend.font_size)
Text:SetPoint(unpack(addon.settings.friend.position))
Text:SetShadowColor(0, 0, 0)
Text:SetShadowOffset(1, -1)
if addon.settings.friend.class_color then
	local _, class = UnitClass('player')
	Text:SetTextColor(RAID_CLASS_COLORS[class].r, RAID_CLASS_COLORS[class].g, RAID_CLASS_COLORS[class].b )
end

local menuFrame = CreateFrame('Frame', addonName..'FriendRightClickMenu', UIParent, 'UIDropDownMenuTemplate')
local menuList = {
	{
		text = OPTIONS_MENU,
		isTitle = true,
...
local function BuildBNTable(total)
	BNTotalOnline = 0
	wipe(BNTable)
	local presenceID, givenName, toonName, toonID, client, isOnline, isAFK, isDND, noteText, realmName, faction, race, class, zoneName, level
	for i = 1, total do
...
Stat:SetScript('OnEnter', function(self)
	...
	if totalonline > 0 then
		GameTooltip:SetOwner(self, unpack(addon.settings.friend.tooltip_position))
